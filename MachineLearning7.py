# -*- coding: utf-8 -*-
"""
Created on Thu Sep  9 07:41:37 2021
...
# theta = tf.Variable(tf.random.uniform(shape = [2, 1], minval=-1, maxval=1),
#                     dtype=tf.float32)
...
#     gradients = g.gradient(mse, [theta]) # mse를 theta에 대해서 미분함
#     theta.assign(theta - learning_rate * gradients[0].numpy())
#     if n_epoch%10 == 0:
#         print(theta)
        
(X_train, y_train), (X_test, y_test) = tf.keras.datasets.mnist.load_data()
nTrain = X_train.shape[0] # 60,000
...
n_epoch = 40
batchSize = 200
nBatch = int(nTrain/batchSize) # 60,000 / 200 = 300
...
#            loss = tf.reduce_mean(tf.keras.losses.MSE(y_batch, y_pred))
    ...
    if epoch % 5 == 0:
        correct = tf.equal(tf.argmax(y_pred, 1), tf.argmax(y_batch, 1)) # 0: 행방향, 1: 열방향으로 최대값을 찾
